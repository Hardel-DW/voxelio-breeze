...
import { collectFromPath } from "./collectFromPath.ts";
import type { IterationResult, IterationValue } from "./type.ts";

export function createIterations(
	valueSet: IterationValue,
	files: Record<string, Uint8Array>,
): IterationResult[] {
	if (
		valueSet.type === "collect_from_path" ||
		valueSet.type === "get_registry_elements"
	) {
		const elements =
			valueSet.type === "collect_from_path"
				? collectFromPath(
